package com.alzimerahmed.omnitype.service
...
import com.alzimerahmed.omnitype.model.CommandType
import com.alzimerahmed.omnitype.ui.processtext.ProcessTextEdit
import com.alzimerahmed.omnitype.ui.processtext.ProcessTextReplacementBridge
import com.alzimerahmed.omnitype.ui.processtext.resolveProcessTextEdit
import com.alzimerahmed.omnitype.R
import com.alzimerahmed.omnitype.OmniTypeApp
...
class AssistantService : AccessibilityService() {
                    ...
                    trigger.endsWith("copy") -> {
                        ...
                        if (textToCopy.isEmpty()) {
                            ...
                            if (wrote) {
                                ...
                                withContext(Dispatchers.Main) {
                                    clipboard.setPrimaryClip(ClipData.newPlainText("OmniType", textToCopy))
                                }
                                performHapticFeedback(HapticFeedbackConstants.CONFIRM)
                    ...
                    trigger.endsWith("cut") -> {
                        ...
                        if (textToCut.isEmpty()) {
                            ...
                            if (wrote) {
                                ...
                                lastUndoSourceId = sourceId(source)
                                withContext(Dispatchers.Main) {
                                    clipboard.setPrimaryClip(ClipData.newPlainText("OmniType", textToCut))
                                }
                                performHapticFeedback(HapticFeedbackConstants.CONFIRM)
     ...
     * OmniType had not copied itself. Writing is not gated, which is why `?copy` works and
     ...
     * anywhere on the device without OmniType ever reading the clipboard.
     *
    ...
    private suspend fun replaceText(
        ...
        val newClip = ClipData.newPlainText("OmniType Result", newText)
        ...
        if (!callerOwnsClipboard) {
            ...
            // the handler, which previously cancelled it and left OmniType's temp clip (the
     ...
     *
     * Leaving OmniType's temp clip in place is not an option — it holds the user's
    ...
    private fun restoreClipboard(clipboard: ClipboardManager, oldClip: ClipData?, ourText: String) {
        try {
            ...
            val recovered = oldClip ?: lastCopiedText?.let { ClipData.newPlainText("OmniType", it) }
     ...
     * +500ms restore and left OmniType's temp clip (the user's transformed text) on the
     * clipboard for good.
     */
